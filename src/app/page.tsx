...
const Portfolio: React.FC = () => {
  ...
  const CERTS_PER_SLIDE = 3;
  const certificationSlides = Array.from(
    { length: Math.ceil(certifications.length / CERTS_PER_SLIDE) },
    (_, i) => certifications.slice(i * CERTS_PER_SLIDE, i * CERTS_PER_SLIDE + CERTS_PER_SLIDE)
  );

  const certsTrackRef = useRef<HTMLDivElement>(null);
  const { scrollYProgress: certsScrollProgress } = useScroll({
    target: certsTrackRef,
    offset: ["start start", "end end"],
  });
  const certsX = useTransform(
    certsScrollProgress,
    [0, 1],
    ["0%", `-${(certificationSlides.length - 1) * 100}%`]
  );
